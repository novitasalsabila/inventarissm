...
<body>
    <div style="padding: 15px;">
        ...
        <hr />
        <form method="get" action="exportpdfAsetUangKeluar.php">
            <div class="row">
            ...
            if(isset($_GET['filter'])) // Jika user mengisi filter tanggal, maka munculkan tombol untuk reset filter
                echo '<a href="exportpdfAsetUangKeluar.php" class="btn btn-default">RESET</a>';
            ?>
        </form>
        <?php
        ...
        if(empty($tgl_awal) or empty($tgl_akhir)){ // Cek jika tgl_awal atau tgl_akhir kosong, maka :
            ...
            $query = "SELECT * FROM tb_asetuangkeluar";
            $url_cetak = "exportpdfAsetUangKeluar.php";
            $label = "Semua Data Transaksi";
        }else{ // Jika terisi
            // Buat query untuk menampilkan data transaksi sesuai periode tanggal
            $query = "SELECT * FROM tb_asetuangkeluar WHERE (tanggalkeluar BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."')";
            $url_cetak = "printuangkeluar.php?tgl_awal=".$tgl_awal."&tgl_akhir=".$tgl_akhir."&filter=true";
                ...
                <tbody>
                    ...
                    if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
                            $tgl = date('d-m-Y', strtotime($data['tanggalkeluar'])); // Ubah format tanggal jadi dd-mm-yyyy
                            echo "<tr>";
                            echo "<td>".$tgl."</td>";
                            echo "<td>".$data['id']."</td>";
                            echo "<td>".$data['nominalkeluar']."</td>";
